...
template <class G, class FC>
inline double cpmByOmp(const G& x, FC fc, double gamma) {
  using K = typename G::key_type;
  ASSERT(gamma > 0);

  size_t S = x.span();

  std::vector<double> vin(S, 0.0);
  std::vector<double> cin(S, 0.0);
  std::vector<double> csize(S, 0.0);

  // 各頂点について、同一コミュニティ内への辺重みを数える
  #pragma omp parallel for schedule(dynamic, 2048)
  for (K u = 0; u < S; ++u) {
    if (!x.hasVertex(u)) continue;
  ...
  #pragma omp parallel for schedule(static) reduction(+:h)
  for (K c = 0; c < S; ++c) {
    double n = csize[c];
    if (n <= 0) continue;

    // cin[c] は対称化グラフでは内部辺を両方向に数えるので 1/2 する
    double e = cin[c] / 2.0;

    h += e - gamma * n * (n - 1.0) / 2.0;
  }
...
inline double deltaCPM(
  ...
  return (vcout - gamma * csize)
       - (vdout - gamma * (dsize - 1.0));
}
#pragma endregion
