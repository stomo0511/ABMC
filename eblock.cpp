 ...
 */
double GiniCoefficient(const std::vector<int>& values)
{
    if(values.empty())
        return 0.0;
    ...
    const std::vector<int>& nodes_per_block)
{
    ...
    double gini = GiniCoefficient(nodes_per_block);
    ...
    int nc)
{
    ...
    double sum_nodes =
        std::accumulate(nodes_per_block.begin(),
                        nodes_per_block.end(),
                        0.0);
    ...
    int max_nodes =
        *std::max_element(nodes_per_block.begin(),
                          nodes_per_block.end());

    int min_nodes =
        *std::min_element(nodes_per_block.begin(),
                          nodes_per_block.end());
    ...
    double mean_of_internal_avg_degrees =
        sum_internal_avg_degree / nb;

    // 4. ブロックグラフの平均次数 (BlockDegreesBinary を使用)
    Graph T =
        BuildBlockGraph(G, block_of, BlockEdgeWeight::Binary);

    std::vector<int> block_degrees =
        BlockDegreesBinary(T);

    double sum_block_degrees =
        std::accumulate(block_degrees.begin(),
                        block_degrees.end(),
                        0.0);

    double avg_block_degree =
        sum_block_degrees / nb;

    // 5. モジュラリティ (Modularity_Unweighted を使用)
    double Q =
        Modularity_Unweighted(G, block_of);
    ...
    std::cout << "3. Internal Avg Degree   : "
              << std::setprecision(4)
              << mean_of_internal_avg_degrees
              << " (Mean of per-block avg)"
              << std::endl;
              
    std::cout << "4. Block Graph Avg Deg   : "
              << avg_block_degree << std::endl;
    
    std::cout << "5. Modularity (Q)        : "
              << std::setprecision(6)
              << Q << std::endl;
